...
public class AlarmServiceTest extends AbstractGatewayTest {
	...
	private int getAlarmCount(String alarmSubjectHref) {
		String count = XmlPath.from(get(alarmSubjectHref).asString()).get(
				"obj.int.@val");
	...
	public void testAlarmSubject() {
		expect().body(hasXPath("/obj[@is='obix:AlarmSubject']"))
	...
	public void testAlarmGeneration() {
		...
		String fanspeedPut = "<obj is='iot:FanSpeedActuator'>"
				...
				+ " <bool name='enabled' href='enabled' val='%b' />" + "</obj>";

		given().body(String.format(fanspeedPut, 1000, false)).put(
				"/testDevicesAlarm/fanSpeed1");
		assertEquals(initialAlarmCount, getAlarmCount());

		given().body(String.format(fanspeedPut, 3000, true)).put(
				"/testDevicesAlarm/fanSpeed1");
		assertEquals(initialAlarmCount, getAlarmCount());

		given().body(String.format(fanspeedPut, 500, true)).put(
				"/testDevicesAlarm/fanSpeed1");
	...
	public void testRepeatedAlarmGeneration() {
		...
		// off normal
		given().body(String.format(fanspeedPut, 1000, true)).put(
				"/testDevicesAlarm/fanSpeed2");
		assertEquals(initialAlarmCount + 1, getAlarmCount());

		// staying off normal
		given().body(String.format(fanspeedPut, 200, true)).put(
				"/testDevicesAlarm/fanSpeed2");
		assertEquals(initialAlarmCount + 1, getAlarmCount());
	...
	public void testAlarmObject() {
		...
		// off normal
		given().body(String.format(fanspeedPut, 1000, true)).put(
				"/testDevicesAlarm/fanSpeed3");
		String alarmHref = getLatestAlarm().get("@href");

		expect().body(
				hasXPath("/obj/ref[@name='source' and @href='/testDevicesAlarm/fanSpeed3']"))
				.body(hasXPath("/obj/abstime[@name='timestamp']")).when()
				.get(alarmHref);
	}
	...
	public void testAckAlarm() {
		...
		source.value().attach(
				new DefaultAlarmObserver(new IntRangeAlarmCondition(10l, 20l))
		...
		expect().body(
				hasXPath("/obj/op[@name='ack' and @href='ack' and @in='obix:AckAlarmIn' and @out='obix:AckAlarmOut']"))
		...
		given().contentType(ContentType.XML)
				.body("<obj is='obix:AckAlarmIn'>"
						+ " <str name='ackUser' val='" + ackUser + "'/>"
						+ "</obj>")
				.expect()
				.body(hasXPath("/obj[@is='obix:AckAlarmOut']"))
				.body(hasXPath("/obj/obj[@href='" + alarmHref + "']"))
				.body(hasXPath("/obj/obj/str[@name='ackUser' and not(@null) and @val='"
						+ ackUser + "']"))
		...
		long ackTimestamp = DatatypeConverter.parseDateTime(
				XmlPath.from(response).getString(
						"obj.abstime.find { it.@name='ackTimestamp' }.@val"))
				.getTimeInMillis();
	...
	public void testAckAlarmWithNullUser() {
		...
		// + "	<str name='ackUser' null='true'/>" + "</obj>")
		given().contentType(ContentType.XML)
				.body("<obj is='obix:AckAlarmIn'>"
						+ "	<str name='ackUser' null='true'/>" + "</obj>")
				.expect()
	...
	public void testStatefulAlarm() {
		...
		expect().body(
				hasXPath("/obj/abstime[@name='normalTimestamp' and @null='true']"))
				.when().get(alarmHref);
		...
		long normalTimestamp = DatatypeConverter
				.parseDateTime(
						XmlPath.from(response)
								.getString(
										"obj.abstime.find { it.@name='normalTimestamp' }.@val"))
				.getTimeInMillis();
		assertEquals(expectedNormalTimestamp, normalTimestamp, 500);

		// changing alarm-state again does not influence recorded normal
		// timestamp
		...
		normalTimestamp = DatatypeConverter
				.parseDateTime(
						XmlPath.from(response)
								.getString(
										"obj.abstime.find { it.@name='normalTimestamp' }.@val"))
	...
	public void testAlarmSourceStatus() {
		...
		val.attach(new DefaultAlarmObserver(
				new IntRangeAlarmCondition(10l, 20l).setFlipped(true), true,
				false).setTarget(source));
		val.attach(new DefaultAlarmObserver(
				new IntRangeAlarmCondition(21l, 30l).setFlipped(true), false,
				true).setTarget(source));
		...
		given().contentType(ContentType.XML)
		.body("<obj is='obix:AckAlarmIn'>"
						+ " <str name='ackUser' val='someUser'/>" + "</obj>")
				.post(alarmHref + "/ack");
	...
	public void testCustomAlarmSubject() {
		AlarmSubjectImpl subject = new AlarmSubjectImpl(
		...
		val.attach(new DefaultAlarmObserver(
				new IntRangeAlarmCondition(10l, 20l).setFlipped(true), true,
				false).setTarget(source).setAlarmSubject(subject));

		int alarms = getAlarmCount();
		val.set(15l);

		assertEquals(alarms, getAlarmCount()); // was not added to default alarm
												// subject
		assertEquals(1, getAlarmCount(subjectHref)); // custom subject has 1
														// alarm registered
	}
	...
	public void testQueryAlarmSubject() throws ParseException {
		...
		alarm10.timestamp().set(dateFormat.parse("2013-09-10 10:00").getTime(),
				tz);
		subject.addAlarm(alarm10);

		AlarmImpl alarm11 = new AlarmImpl(source, true, false);
		alarm11.timestamp().set(dateFormat.parse("2013-09-10 11:00").getTime(),
				tz);
		subject.addAlarm(alarm11);

		AlarmImpl alarm12 = new AlarmImpl(source, false, true);
		alarm12.timestamp().set(dateFormat.parse("2013-09-10 12:00").getTime(),
				tz);
		subject.addAlarm(alarm12);

		AlarmImpl alarm13 = new AlarmImpl(source, false, false);
		alarm13.timestamp().set(dateFormat.parse("2013-09-10 13:00").getTime(),
				tz);
		subject.addAlarm(alarm13);

		// query the alarm subject
		expect().body(hasXPath("/obj[@is='obix:AlarmQueryOut']"))
		...
		given().contentType(ContentType.XML)
				...
				.body(hasXPath("/obj/abstime[@name='end']"))
				.body(hasXPath("/obj/list[@name='data' and @of='obix:Alarm']"))
				.body(hasXPath("/obj/list/obj[@href='"
						+ alarm11.getFullContextPath() + "']"))
				.body(hasXPath("/obj/list/obj[@href='"
						+ alarm12.getFullContextPath() + "']")).when()
				.post(subjectHref + "/query");

		// limit to 2 alarms
		given().contentType(ContentType.XML)
				.body("<obj is='obix:AlarmFilter'>"
						+ "  <int name='limit' val='2'/>" + "</obj>")
				.expect()
				.body(hasXPath("/obj[@is='obix:AlarmQueryOut']"))
				.body(hasXPath("/obj/int[@name='count' and @val='2']"))
				.body(hasXPath("/obj/abstime[@name='start']"))
				...
				.body(hasXPath("/obj/list[@name='data' and @of='obix:Alarm']"))
				.body(hasXPath("/obj/list/obj[@href='"
						+ alarm10.getFullContextPath() + "']"))
				.body(hasXPath("/obj/list/obj[@href='"
						+ alarm11.getFullContextPath() + "']")).when()
				.post(subjectHref + "/query");
	}
	...
	public void testQueryAlarmSource() {
		...
		val1.attach(new DefaultAlarmObserver(new IntRangeAlarmCondition(10l,
				20l).setFlipped(true), false, false).setTarget(source1));

		BrightnessActuatorImpl source2 = new BrightnessActuatorImpl();
		source2.setHref(new Uri(source2href));
		Int val2 = source2.value();
		val2.attach(new DefaultAlarmObserver(new IntRangeAlarmCondition(10l,
				20l).setFlipped(true), false, false).setTarget(source2));
		...
		given().contentType(ContentType.XML)
				...
				.body(hasXPath("/obj/abstime[@name='end']"))
				.body(hasXPath("/obj/list[@name='data' and @of='obix:Alarm']"))
				.body(hasXPath("/obj/list/obj/ref[@name='source' and @href='"
						+ source1.getFullContextPath() + "']"))
				.body(not(hasXPath("/obj/list/obj/ref[@name='source' and @href='"
						+ source2.getFullContextPath() + "']"))).when()
				.post("/alarms/query");
	}
	...
	public void testQueryUnackedAlarms() {
		...
		expect().body(hasXPath("/obj[@is='obix:AlarmQueryOut']"))
				...
				.body(hasXPath("/obj/list[@name='data' and @of='obix:Alarm']"))
				.body(hasXPath("/obj/list/obj[@href='"
						+ alarmUnacked.getFullContextPath() + "']"))
				.body(hasXPath("/obj/list/obj[@href='"
						+ alarmStateful.getFullContextPath() + "']"))
				.body(hasXPath("/obj/list/obj[@href='"
						+ alarmAcked.getFullContextPath() + "']")).when()
				.post(subjectHref + "/query");

		// unacked = true, return unacked AckAlarms
		given().contentType(ContentType.XML)
				.body("<obj is='obix:AlarmFilter'>"
						+ "  <bool name='unacked' val='true'/>" + "</obj>")
				.expect()
				.body(hasXPath("/obj[@is='obix:AlarmQueryOut']"))
				.body(hasXPath("/obj/int[@name='count' and @val='1']"))
				.body(hasXPath("/obj/abstime[@name='start']"))
				...
				.body(hasXPath("/obj/list/obj[@href='"
						+ alarmUnacked.getFullContextPath() + "']"))
				.body(not(hasXPath("/obj/list/obj[@href='"
						+ alarmStateful.getFullContextPath() + "']")))
				.body(not(hasXPath("/obj/list/obj[@href='"
						+ alarmAcked.getFullContextPath() + "']"))).when()
				.post(subjectHref + "/query");
		...
		given().contentType(ContentType.XML)
				.body("<obj is='obix:AlarmFilter'>"
						+ "  <bool name='unacked' val='false'/>" + "</obj>")
				.expect()
				.body(hasXPath("/obj[@is='obix:AlarmQueryOut']"))
				.body(hasXPath("/obj/int[@name='count' and @val='1']"))
				.body(hasXPath("/obj/abstime[@name='start']"))
	...
	public void testQueryActiveAlarms() {
		...
		Int val1 = source1.value();
		val1.attach(new DefaultAlarmObserver(new IntRangeAlarmCondition(10l,
				20l).setFlipped(true), true, false).setTarget(source1)
				.setAlarmSubject(subject));

		BrightnessActuatorImpl source2 = new BrightnessActuatorImpl();
		source2.setHref(new Uri(source2href));
		Int val2 = source2.value();
		val2.attach(new DefaultAlarmObserver(new IntRangeAlarmCondition(10l,
				20l).setFlipped(true), true, false).setTarget(source2)
				.setAlarmSubject(subject));
		...
		// query the alarm subject for active alarms (StatefulAlarms that have
		// not returned to normal)
		given().contentType(ContentType.XML)
				.body("<obj is='obix:AlarmFilter'>"
						+ "  <bool name='active' val='true'/>" + "</obj>")
				.expect()
				.body(hasXPath("/obj[@is='obix:AlarmQueryOut']"))
				.body(hasXPath("/obj/int[@name='count' and @val='1']"))
				.body(hasXPath("/obj/list[@name='data' and @of='obix:Alarm']"))
		...
		given().contentType(ContentType.XML)
				.body("<obj is='obix:AlarmFilter'>"
						+ "  <bool name='active' val='false'/>" + "</obj>")
				.expect()
				.body(hasXPath("/obj[@is='obix:AlarmQueryOut']"))
				.body(hasXPath("/obj/int[@name='count' and @val='1']"))
				.body(hasXPath("/obj/list[@name='data' and @of='obix:Alarm']"))
