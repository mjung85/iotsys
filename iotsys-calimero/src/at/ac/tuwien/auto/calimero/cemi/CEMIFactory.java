...
package at.ac.tuwien.auto.calimero.cemi;
...
public final class CEMIFactory
{
	 ...
	 *         cEMI message code
	 */
	public static CEMI create(byte[] data, int offset, int length)
		throws KNXFormatException
	{
		...
		default:
			throw new KNXFormatException("not supported cEMI msg code", mc);
		}
	}
	 ...
	 *         failed
	 */
	public static CEMI create(int msgCode, byte[] data, CEMI original)
		throws KNXFormatException
	{
		switch (msgCode) {
		case CEMILData.MC_LDATA_REQ:
		...
		case CEMIBusMon.MC_BUSMON_IND:
			...
			return new CEMIBusMon(f.getStatus(), f.getTimestamp(),
	 ...
	 * @return the new cEMI message adjusted with KNX addresses
	 */
	public static CEMI create(IndividualAddress src, KNXAddress dst,
		CEMILData original, boolean extended)
	{
		return create(0, src, dst, null, original, extended);
	}
	...
	private static byte[] truncate(byte[] data, int offset, int len)
	{
