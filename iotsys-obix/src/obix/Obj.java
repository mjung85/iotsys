...
public class Obj implements IObj, Subject, AlarmSource, Cloneable
{
	...
	public void notifyObservers()
	{
		...
		synchronized (observers)
		{
			...
			{
				iterator.next().update(getCurrentState());
			}
		}
	...
	/**
	 * Returns a reference to the Obj with/without absolute Href
	 */
	@Override
	...
	public synchronized Obj add(Obj kid, boolean checkDuplicates)
	{
		...
		{
			if (kid.getName() != null && kidsByName != null && kidsByName.containsKey(kid.getName()) && !kidsByName.get(kid.getName()).isHidden())
				throw new IllegalStateException("Duplicate child name '" + kid.getName() + "'");
		}

		// if named and key is not contained, add to name map
		if (kid.getName() != null && !kidsByName.containsKey(kid.getName()))
		{
			kidsByName.put(kid.getName(), kid);
		}
	...
	public synchronized void replace(Obj oldObj, Obj newObj)
	{
		...
		if (!oldObj.getName().equals(newObj.getName()))
			throw new IllegalStateException("Mismatched names: " + oldObj.getName() + " != " + newObj.getName());

		// sanity checks
		if (oldObj.getParent() != this)
		...
		// replace in map
		kidsByName.put(newObj.getName(), newObj);
	...
	private static HashMap<String, Class<?>> elemNameToClass = new HashMap<String, Class<?>>();
	private static HashMap<Integer, Class<?>> binCodeToClass = new HashMap<Integer, Class<?>>();
