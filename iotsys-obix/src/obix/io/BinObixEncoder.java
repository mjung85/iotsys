...
/**
 * BinObixEncoder is used to serialize an Obj tree to an binary object stream.
 * 
 * @author Brian Frank
 * @creation 1 Sep 09
 * @version $Revision$ $Date$
 */
public class BinObixEncoder extends DataOutputStream
{
	...
	{
		return toBytes(obj, Obj.DEFAULT_LANGUAGE);
	}

	/**
	 * Encode the specified obj to an in memory byte array using localized strings.
	 */
	public static byte[] toBytes(Obj obj, String language)
	{
		try
		{
	 ...
	 */
	public BinObixEncoder(OutputStream out) throws IOException
	{
		super(out);
	}
	...
	public void encode(Obj obj, String language) throws IOException
	{
		...
		Node node = toNode(obj, hasChildren, language);
		while (node != null)
		{
			node.write(this);
			node = node.next;
	...
	private Node toNode(Obj obj, boolean markTailMore, String language)
	{
		...
		switch (objCode)
		{
			...
			case BinObix.BOOL:
				...
				if (b.getRange() != null)
					tail = new StrNode(tail, BinObix.RANGE, b.getRange().toString());
				break;
			...
			case BinObix.INT:
				...
				if (i.getMin() != Int.MIN_DEFAULT)
					tail = new IntNode(tail, BinObix.MIN, i.getMin());
				if (i.getMax() != Int.MAX_DEFAULT)
					tail = new IntNode(tail, BinObix.MAX, i.getMax());
				if (i.getUnit() != null)
					tail = new StrNode(tail, BinObix.UNIT, i.getUnit().toString());
				break;

			case BinObix.REAL:
				Real f = (Real) obj;
				head = tail = new RealNode(null, objCode, f.get());
				if (f.getMin() != Real.MIN_DEFAULT)
					tail = new RealNode(tail, BinObix.MIN, f.getMin());
				if (f.getMax() != Real.MAX_DEFAULT)
					tail = new RealNode(tail, BinObix.MAX, f.getMax());
				if (f.getUnit() != null)
					tail = new StrNode(tail, BinObix.UNIT, f.getUnit().toString());
				if (f.getPrecision() != Real.PRECISION_DEFAULT)
					tail = new IntNode(tail, BinObix.PRECISION, f.getPrecision());
				break;
			...
			case BinObix.STR:
				...
				if (s.getMin() != Str.MIN_DEFAULT)
					tail = new IntNode(tail, BinObix.MIN, s.getMin());
				if (s.getMax() != Str.MAX_DEFAULT)
					tail = new IntNode(tail, BinObix.MAX, s.getMax());
				break;
			...
			case BinObix.ENUM:
				...
				if (e.getRange() != null)
					tail = new StrNode(tail, BinObix.RANGE, e.getRange().toString());
				break;
			...
			case BinObix.ABSTIME:
				...
				if (a.getMin() != null)
					tail = new AbstimeNode(tail, BinObix.MIN, a.getMin());
				if (a.getMax() != null)
					tail = new AbstimeNode(tail, BinObix.MAX, a.getMax());
				if (a.getTz() != null)
					tail = new StrNode(tail, BinObix.TZ, a.getTz());
				break;
			...
			case BinObix.RELTIME:
				...
				if (r.getMin() != null)
					tail = new ReltimeNode(tail, BinObix.MIN, r.getMin());
				if (r.getMax() != null)
					tail = new ReltimeNode(tail, BinObix.MAX, r.getMax());
				break;
			...
			case BinObix.TIME:
				...
				if (t.getMin() != null)
					tail = new TimeNode(tail, BinObix.MIN, t.getMin());
				if (t.getMax() != null)
					tail = new TimeNode(tail, BinObix.MAX, t.getMax());
				if (t.getTz() != null)
					tail = new StrNode(tail, BinObix.TZ, t.getTz());
				break;
			...
			case BinObix.DATE:
				...
				if (d.getMin() != null)
					tail = new DateNode(tail, BinObix.MIN, d.getMin());
				if (d.getMax() != null)
					tail = new DateNode(tail, BinObix.MAX, d.getMax());
				if (d.getTz() != null)
					tail = new StrNode(tail, BinObix.TZ, d.getTz());
				break;
			...
			case BinObix.LIST:
				...
				if (!list.getOf().containsOnlyObj())
					tail = new ContractNode(tail, BinObix.OF, list.getOf());
				if (list.getMin() != List.MIN_DEFAULT)
					tail = new IntNode(tail, BinObix.MIN, list.getMin());
				if (list.getMax() != List.MAX_DEFAULT)
					tail = new IntNode(tail, BinObix.MAX, list.getMax());
				break;
			...
			case BinObix.OP:
				...
				if (!op.getIn().containsOnlyObj())
					tail = new ContractNode(tail, BinObix.IN, op.getIn());
				if (!op.getOut().containsOnlyObj())
					tail = new ContractNode(tail, BinObix.OUT, op.getOut());
				break;
			...
			case BinObix.FEED:
				...
				if (!feed.getIn().containsOnlyObj())
					tail = new ContractNode(tail, BinObix.IN, feed.getIn());
				if (!feed.getOf().containsOnlyObj())
					tail = new ContractNode(tail, BinObix.OF, feed.getOf());
				break;
		...
		// object facets
		if (obj.getName() != null)
			tail = new StrNode(tail, BinObix.NAME, obj.getName());
		if (obj.getHref() != null)
			tail = new StrNode(tail, BinObix.HREF, obj.getHref().encodeVal());
		if (obj.getIs() != null)
			tail = new ContractNode(tail, BinObix.IS, obj.getIs());
		if (obj.getStatus() != Status.ok)
			tail = new StatusNode(tail, BinObix.STATUS_0, obj.getStatus());
		if (obj.getDisplay() != null)
			tail = new StrNode(tail, BinObix.DISPLAY, obj.getTranslation(language, TranslationAttribute.display));
		if (obj.getDisplayName() != null)
			tail = new StrNode(tail, BinObix.DISPLAY_NAME, obj.getTranslation(language, TranslationAttribute.displayName));
		if (obj.getIcon() != null)
			tail = new StrNode(tail, BinObix.ICON, obj.getIcon().encodeVal());
		if (obj.isNull())
			tail = new BoolNode(tail, BinObix.NULL, true);
		if (obj.isWritable())
			tail = new BoolNode(tail, BinObix.WRITABLE, true);
	...
	static class StatusNode extends Node
	{
		...
		void write(DataOutputStream out) throws IOException
		{
			...
			if (val == Status.disabled)
				x = BinObix.STATUS_0 | BinObix.STATUS_0_DISABLED;
			else if (val == Status.fault)
				x = BinObix.STATUS_0 | BinObix.STATUS_0_FAULT;
			else if (val == Status.down)
				x = BinObix.STATUS_0 | BinObix.STATUS_0_DOWN;
			else if (val == Status.unackedAlarm)
				x = BinObix.STATUS_0 | BinObix.STATUS_0_UNACKED_ALARM;
			else if (val == Status.alarm)
				x = BinObix.STATUS_1 | BinObix.STATUS_1_ALARM;
			else if (val == Status.unacked)
				x = BinObix.STATUS_1 | BinObix.STATUS_1_UNACKED;
			else if (val == Status.overridden)
				x = BinObix.STATUS_1 | BinObix.STATUS_1_OVERRIDDEN;
			else
	...
	private HashMap<String, Integer> strTable = new HashMap<String, Integer>(); // String -> Integer
}
